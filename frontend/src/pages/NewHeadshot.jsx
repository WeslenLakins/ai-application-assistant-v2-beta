...
function NewHeadshot() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const imageUrlsArray = await Promise.all(imageUrls.map((image) => storeImage(image)));
      ...
      toast.success("Workflow run successfully initiated and return data saved to database!");
    } catch (error) {
    ...
    <div className="new-headshot-container">
      ...
      <main>
        <form onSubmit={onSubmit}>
          ...
          {/* File Upload */}
          <div className="form-group">
            <label className="formLabel">Upload 4 Images</label>
            <input type="file" id="images" onChange={onMutate} max='4' min='4' accept=".jpg,.png,.jpeg" multiple required />
          </div>
          <div>
            <button className="primaryButton" type="submit">Create Headshot</button>
          </div>
        </form>
      </main>
    </div>
